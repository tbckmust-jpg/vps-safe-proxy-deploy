...
新版本 Xray 已移除 HY2 自签场景里旧的 `allowInsecure` 跳过校验字段。自签模式下 credentials 会额外写入证书 pin、原生 Hysteria `pinSHA256`、`hy2_uri`、以及 v2rayN/Xray 专用 pinned URI；v2rayN 中如果普通 URI 仍触发 Core 启动失败，优先尝试 `v2rayN_xray_uri` 或 sing-box outbound。
